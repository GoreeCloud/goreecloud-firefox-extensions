...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.11": fail("source version must remain synchronized with canonical inventory")
    ...
    if background.get("scripts") != ["src/background.js","src/routing-settings-background.js","src/commands-background.js","src/isolation-health-background.js"]: fail("0.1.11 background modules are not registered in the required order")
    expected_commands={"open-webspaces-launcher","open-standard-webspace","open-goreecloud-webspace","open-google-webspace","open-microsoft-webspace","open-meta-webspace","toggle-routing-pause","open-webspaces-manager"}
    ...
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config","routing-settings-ui.js","shortcuts-ui.js","shortcuts.css","isolation-health-panel","isolation-health-ui.js","isolation-health.css","0.1.11 source candidate"]:
        if token not in options: fail(f"manager is missing required 0.1.11 control or module: {token}")
    ...
    if 'STANDARD_WEBSPACE_ID = "standard"' not in constants or 'name: "Standard"' not in constants: fail("0.1.11 must retain the built-in Standard Webspace")
    ...
    print("Validated GoreeCloud Webspaces 0.1.11 Standard-fallback, per-Webspace isolation, and isolation-health runtime-message source candidate.")
