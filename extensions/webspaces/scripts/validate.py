...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.7": fail("source version must remain synchronized with canonical inventory")
    ...
    if background.get("scripts") != ["src/background.js","src/routing-settings-background.js"]: fail("0.1.7 routing-control background module is not registered in the required order")
    required_files=["README.md","SPECIFICATIONS.md","FEATURES.md","BENEFITS.md","COMPETITIVE-OBJECTIVES.md","SECURITY.md","PRIVACY.md","LICENSE","icons/webspaces.svg","src/background.js","src/constants.js","src/containers.js","src/context-menus.js","src/lifecycle.js","src/management.js","src/portability.js","src/provider-rules.js","src/routing.js","src/routing-controls.js","src/routing-settings-background.js","src/bulk-rules.js","src/storage.js","src/tab-migration.js","ui/glaze-webspaces.css","ui/identity.js","ui/popup.html","ui/popup.css","ui/popup.js","ui/options.html","ui/options.css","ui/options.js","ui/routing-controls.css","ui/routing-controls-ui.js","ui/routing-settings-ui.js","tests/routing.test.js","tests/routing-explain.test.js","tests/management.test.js","tests/management-expanded.test.js","tests/tab-migration.test.js","tests/context-menus.test.js","tests/lifecycle.test.js","tests/portability.test.js","tests/glaze-ui.test.js","tests/identity-ui.test.js","tests/ui-capabilities.test.js","tests/locked-assignment-guard.test.js","tests/routing-controls.test.js"]
    ...
    for token in ["why-panel","new-temporary","move-tab","close-forget","routing-controls-ui.js"]:
        if token not in popup: fail(f"popup is missing required 0.1.7 control or module: {token}")
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config","routing-settings-ui.js","0.1.7 source candidate"]:
        if token not in options: fail(f"manager is missing required 0.1.7 control or module: {token}")
    ...
    print("Validated GoreeCloud Webspaces 0.1.7 routing-control and bulk-assignment source candidate.")
