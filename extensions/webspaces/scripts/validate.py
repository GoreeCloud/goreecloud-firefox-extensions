...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.10": fail("source version must remain synchronized with canonical inventory")
    ...
    if background.get("scripts") != ["src/background.js","src/routing-settings-background.js","src/commands-background.js","src/isolation-health-background.js"]: fail("0.1.10 background modules are not registered in the required order")
    ...
    for command in manifest.get("commands",{}).values():
        if command.get("suggested_key"): fail("0.1.10 must leave shortcut assignment under explicit Firefox/user control")
    ...
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config","routing-settings-ui.js","shortcuts-ui.js","shortcuts.css","isolation-health-panel","isolation-health-ui.js","isolation-health.css","0.1.10 source candidate"]:
        if token not in options: fail(f"manager is missing required 0.1.10 control or module: {token}")
    ...
    if 'STANDARD_WEBSPACE_ID = "standard"' not in constants or 'name: "Standard"' not in constants: fail("0.1.10 must retain the built-in Standard Webspace")
    ...
    if "webspaces:get-isolation-health" not in isolation_background: fail("isolation health background endpoint is required")
    ...
    print("Validated GoreeCloud Webspaces 0.1.10 Standard-fallback, per-Webspace isolation, and isolation-health source candidate.")
