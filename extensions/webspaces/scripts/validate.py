...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.6": fail("source version must remain synchronized with canonical inventory")
    ...
    for token in ["why-panel","new-temporary","move-tab","close-forget"]:
        if token not in popup: fail(f"popup is missing required 0.1.6 control: {token}")
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config"]:
        if token not in options: fail(f"manager is missing required 0.1.6 control: {token}")
    print("Validated GoreeCloud Webspaces 0.1.6 source-candidate runtime-hardening slice.")
