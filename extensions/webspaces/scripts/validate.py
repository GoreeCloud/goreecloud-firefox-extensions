...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.9": fail("source version must remain synchronized with canonical inventory")
    ...
    if background.get("scripts") != ["src/background.js","src/routing-settings-background.js","src/commands-background.js"]: fail("0.1.9 background modules are not registered in the required order")
    ...
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config","routing-settings-ui.js","shortcuts-ui.js","shortcuts.css","0.1.9 source candidate"]:
        if token not in options: fail(f"manager is missing required 0.1.9 control or module: {token}")
