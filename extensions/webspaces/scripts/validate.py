...
def main() -> None:
    ...
    if manifest.get("version") != "0.1.12": fail("source version must remain synchronized with canonical inventory")
    ...
    if background.get("scripts") != ["src/background.js","src/routing-settings-background.js","src/commands-background.js","src/isolation-health-background.js"]: fail("0.1.12 background modules are not registered in the required order")
    ...
    for token in ["add-assignment","assignment-search","routing-tester","export-config","import-config","routing-settings-ui.js","shortcuts-ui.js","shortcuts.css","isolation-health-panel","isolation-health-ui.js","isolation-health.css","provider-logos.js","0.1.12 source candidate"]:
        if token not in options: fail(f"manager is missing required 0.1.12 control or module: {token}")
    ...
    if 'STANDARD_WEBSPACE_ID = "standard"' not in constants or 'name: "Standard"' not in constants: fail("0.1.12 must retain the built-in Standard Webspace")
    if 'id: "proton"' not in constants or 'name: "Proton"' not in constants: fail("0.1.12 must include the built-in Proton Webspace")
    ...
    print("Validated GoreeCloud Webspaces 0.1.12 provider-branding, Proton routing, Standard-fallback, per-Webspace isolation, and isolation-health source candidate.")
