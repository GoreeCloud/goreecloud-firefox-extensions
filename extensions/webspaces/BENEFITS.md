...
Every managed Webspace is required to own a distinct Firefox contextual identity and unique cookie store. That gives Standard, provider Webspaces, custom persistent Webspaces, duplicated Webspaces, and temporary Webspaces independent cookie-backed login state instead of intentionally sharing one authenticated browser identity. The implementation also refuses duplicate cookie-store mappings so a configuration error cannot silently collapse two Webspaces into the same Firefox container.
