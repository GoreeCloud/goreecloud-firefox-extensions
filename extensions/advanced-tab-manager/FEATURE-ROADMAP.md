...
| ATM-008F | Refine popup, sidebar, and Manager visual hierarchy/interaction quality; remove stale embedded lifecycle labels; requalify the material presentation delta against current Stable Glaze UI. | High | Implemented in 0.1.12 source candidate. Exact-head repository/runtime/release/security/Glaze source gates pass; owner-rendered Firefox 156/Linux normal-light review verifies warning removal, single-row sidebar toolbar, Manager snapshot retention/empty state and balanced 3×2 metrics, and popup 2×2 metrics. Keyboard/assistive-technology, Forced Colors, Reduced Transparency, dark appearance, signing, and Stable acceptance remain pending. |
