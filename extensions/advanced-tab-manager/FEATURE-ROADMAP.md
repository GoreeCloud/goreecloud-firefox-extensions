...
| ATM-008F | Refine popup, sidebar, and Manager visual hierarchy/interaction quality; remove stale embedded lifecycle labels; requalify the material presentation delta against current Stable Glaze UI. | High | Implemented in 0.1.12 source candidate; current exact-head Firefox Repository, real-Firefox runtime, release qualification, security, and Glaze source/machine gates pass. Owner-rendered Firefox review has verified the MV3 warning removal and single-row sidebar toolbar; snapshot rendering, native tab-activation semantics, balanced Manager metrics, popup metric readability, broader rendered accessibility/appearance review, signing, and Stable acceptance remain pending. |
