#!/usr/bin/env python3
"""Verify a Mozilla-signed Advanced Tab Manager XPI against the deterministic candidate."""
...
def verify_manifest(unsigned_manifest: dict, signed_manifest: dict) -> str:
    ...
    if unsigned_dcp is not None:
        if signed_dcp != unsigned_dcp:
            raise SystemExit("Mozilla-signed manifest changed source-declared data_collection_permissions")
...
def main() -> int:
    ...
    target = Path(f"dist/goreecloud-advanced-tab-manager-{version}-signed.xpi")
    ...
    with zipfile.ZipFile(candidate) as unsigned_archive, zipfile.ZipFile(target) as signed_archive:
        ...
        if signed_payload_names != unsigned_payload_names:
            raise SystemExit(
                "Mozilla-signed XPI payload inventory differs from deterministic candidate: "
