#!/usr/bin/env python3
"""Recover an already-approved unlisted Advanced Tab Manager AMO version."""
...
ADDON_ID = "advanced-tab-manager@goreecloud.com"
AMO_ORIGIN = "https://addons.mozilla.org"
USER_AGENT = "GoreeCloud-Advanced-Tab-Manager-Signing-Acceptance/1.0"
...
def read_authenticated_signed_file(file_id: int, version: str, token: str) -> tuple[bytes, str | None, str]:
    filename = f"goreecloud-advanced-tab-manager-{version}.xpi"
...
def main() -> int:
    ...
    signed_bytes, target_digest, download_api_url = read_authenticated_signed_file(file_id, version, token)
