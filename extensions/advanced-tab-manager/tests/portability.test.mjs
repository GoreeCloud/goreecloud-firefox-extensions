...
test("backup bundle is canonical, versioned, identity-bound, and integrity-checked", async () => {
  const bundle = await createBackupBundle({ extensionVersion: "0.1.9", exportedAt: 123, ...states }, { digestHex });
...
test("import preview reports replacement scope without browsing content", () => {
  const preview = buildImportPreview({
    current: {
      organizational: { revision: 8, tabSets: [{ id: "set-a" }, { id: "set-b" }], stashedItems: [{ id: "stash-a" }] },
      snooze: { revision: 9, items: [{ id: "snooze-a" }] },
      rules: { revision: 10, rules: [{ id: "rule-a" }] }
    },
    imported: {
      organizational: { revision: 2, tabSets: [{ id: "set-a" }], stashedItems: [{ id: "stash-z" }] },
  ...
  assert.deepEqual(preview.importedCounts, { tabSets: 1, stashed: 1, snoozed: 2, rules: 1 });
  assert.deepEqual(preview.conflictCounts, { tabSets: 1, stashed: 0, snoozed: 1, rules: 0 });
