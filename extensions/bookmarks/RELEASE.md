...
3. Firefox runtime acceptance confirms current-page capture, collection, tags, notes, authentication failure, duplicate handling, multi-user isolation, and token-revocation behavior using `RUNTIME_ACCEPTANCE.md`.
...
7. Post-restart capture and token-revocation behavior is revalidated.
8. The exact signed artifact and source revision are recorded without committing signing credentials.

For version 0.1.1, source validation and the server-side extension-capture contract are integrated. The extension remains an `authenticated-source-candidate`; real Firefox runtime and signed-artifact gates remain mandatory.
