...
- Former `extensions/download-manager/` → `GoreeCloud/goreecloud-advanced-download-manager/clients/firefox/`: application-owned Firefox client migration completed on 2026-09-18 via Advanced Download Manager PR #14. The owning application repository is authoritative; the transitional shared-repository copy and shared signing workflow are removed.
